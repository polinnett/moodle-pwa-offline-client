...
export const BookContent = ({
  ...
  useEffect(() => {
    const structureContent = module.contents?.find(
      (c) => c.filename === "structure",
    );
    if (!structureContent?.content) return;

    const structure = JSON.parse(structureContent.content) as {
      title: string;
      href: string;
    }[];
    const chapterFiles =
    ...
    getOfflineLesson(module.id).then(async (saved) => {
      if (saved?.html) {
        ...
        if (!isNaN(chapterCount)) {
          ...
          const chapterData = await getLesson(
            module.id * 1000 + currentChapter,
          );
          if (chapterData?.html) {
    ...
    <div className="space-y-4">
      ...
      {chapters.length > 1 && (
        <div className="flex gap-3">
          ...
          <button
            onClick={() =>
              setCurrentChapter((p) => Math.min(chapters.length - 1, p + 1))
            }
