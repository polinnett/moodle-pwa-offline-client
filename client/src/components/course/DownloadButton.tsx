...
export const DownloadButton = ({
  ...
  const cacheModule = async (module: CourseModule) => {
    ...
    if (module.modname === "page") {
      ...
      if (existing) return;
      const htmlFile = module.contents?.find(
    ...
    if (module.modname === "resource") {
      ...
      const blob = await res.blob();
      const mimeType =
        file.mimetype === "video/mp4" ? "video/mp4" : "application/pdf";
      const cacheName =
        file.mimetype === "video/mp4" ? "moodle-videos" : "moodle-files";
    ...
    if (module.modname === "book") {
      ...
      if (existing) return;
      const chapters =
        module.contents?.filter((c) => c.filename === "index.html") ?? [];
      const { saveLessonOffline } = await import("../../db");
    ...
    if (module.modname === "forum") {
      ...
      const { saveLessonOffline } = await import("../../db");
      try {
        const { getForumsByCourse, getForumDiscussions } =
          await import("../../api/moodle");
        const forums = await getForumsByCourse(courseId);
        const found = forums.find(
          (f: { cmid: number }) => f.cmid === module.id,
        );
        if (found) {
    ...
    try {
      ...
      for (const module of allModules) {
        try {
          ...
          /* пропускаем модуль если не удалось скачать */
        }
      }
    ...
    try {
      ...
      for (const module of allModules) {
        ...
        try {
          ...
          /* модуль мог не быть сохранен офлайн */
        }
