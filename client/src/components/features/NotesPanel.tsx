...
export const NotesPanel = ({
  ...
  const loadOfflineNotes = () => {
    ...
    return stored
      ? (JSON.parse(stored) as Note[]).filter((n) => !n.is_deleted)
      : [];
  };
  ...
  const loadNotes = async () => {
    if (isOnline) {
      try {
        ...
        const allNotes = [
          ...serverNotes.map((n) => ({ ...n, synced: true })),
          ...localUnsync,
        ];
    ...
    if (isOnline) {
      try {
        ...
        localStorage.setItem(
          offlineKey,
          JSON.stringify([...existing, { ...created, synced: true }]),
        );
  ...
  const handleDelete = async (note: Note) => {
    ...
    if (localId && !noteId) {
      const stored = localStorage.getItem(offlineKey);
      if (!stored) return;
      const updated = (JSON.parse(stored) as Note[]).filter(
        (n) => n.localId !== localId,
      );
      localStorage.setItem(offlineKey, JSON.stringify(updated));
  ...
  return (
    <>
      <div
        className="hidden md:flex w-64 shrink-0 rounded-2xl border border-green-200
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                    ...
                    <div className="flex gap-1.5">
                      <button
                        ...
                        className="flex-1 py-1 rounded-lg text-xs font-medium bg-green-500 text-white hover:bg-green-600 cursor-pointer transition-colors"
                      >
                        Сохранить
                      </button>
                      <button
                        onClick={() => setEditingId(null)}
                        className="flex-1 py-1 rounded-lg text-xs font-medium bg-gray-100 dark:bg-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-200 cursor-pointer transition-colors"
                      >
                        Отмена
                      </button>
                    </div>
